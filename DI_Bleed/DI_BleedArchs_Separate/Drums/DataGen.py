####################################################################################################
#                         DRIVER TO GENERATE INPUT DATA (DRUMS: MUSDB18HQ)                         #
####################################################################################################
...
for i in tqdm(sorted(os.listdir(path))): #INPUT NO OF FILES AT ONCE
    ...
    bleed, fs = lb.load(path+i+'/drums.wav', sr=22050)
    true, fs = lb.load(org_path+i+'/drums.wav', sr=22050)
...
np.save('/home/anchal/Desktop/rajesh/DI_Bleed_Archs/Separate/Drums/xtest_drums.npy', xtrain)
np.save('/home/anchal/Desktop/rajesh/DI_Bleed_Archs/Separate/Drums/ytest_drums.npy', ytrain)
