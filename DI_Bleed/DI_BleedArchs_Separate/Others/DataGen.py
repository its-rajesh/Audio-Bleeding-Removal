####################################################################################################
#                         DRIVER TO GENERATE INPUT DATA (OTHERS: MUSDB18HQ)                         #
####################################################################################################
...
for i in tqdm(sorted(os.listdir(path))): #INPUT NO OF FILES AT ONCE
    ...
    bleed, fs = lb.load(path+i+'/other.wav', sr=22050)
    true, fs = lb.load(org_path+i+'/other.wav', sr=22050)
    
    stft_bleed = lb.stft(bleed, hop_length = 2048)
    stft_true = lb.stft(true, hop_length = 2048)
    
    encode_bleed = dp.DynamicInput.encode(dp.DynamicInput, stft_bleed, 3, 1)
    encode_true = dp.DynamicInput.encode(dp.DynamicInput, stft_true, 3, 1)
...
np.save('/home/anchal/Desktop/rajesh/DI_Bleed_Archs/Separate/Others/xtest_other.npy', xtrain)
np.save('/home/anchal/Desktop/rajesh/DI_Bleed_Archs/Separate/Others/ytest_other.npy', ytrain)
